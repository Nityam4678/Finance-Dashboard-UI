...
// SVG Icons
const RewardsIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-4 h-4 text-primary-500">
      ...
      <Card.Content className="py-2">
        <div className="flex justify-around">
          {actions.map((action) => (
            <button
              key={action.label}
              className="flex flex-col items-center gap-0.5 p-1.5 rounded-xl hover:bg-dark-300 transition-smooth"
            >
              <div className="w-8 h-8 rounded-lg bg-dark-300 flex items-center justify-center">
                <action.icon />
              </div>
              <span className={`text-[10px] ${action.active ? 'text-primary-500' : 'text-gray-400'}`}>{action.label}</span>
            </button>
          ))}
        </div>
      </Card.Content>
